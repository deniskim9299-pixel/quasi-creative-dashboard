...
function DateRangePicker({ bounds, start, end, setStart, setEnd }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, padding: "10px 14px", background: "#fff", border: "1px solid #E8DCD0", borderRadius: 8, flexWrap: "wrap" }}>
      ...
      <span title="Filters ads by first-seen date — the earliest day Triple Whale reported spend for the ad. This is the creative's birthdate — not the activity window. Spend/revenue shown are lifetime totals for ads created in this range."
        style={{ marginLeft: "auto", fontSize: 10, color: "#b0978f", cursor: "help", fontStyle: "italic" }}>
        ⓘ Filters by first-seen date, not activity — spend shown is lifetime for ads in this range
      </span>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`https://raw.githubusercontent.com/${GITHUB_REPO}/main/data/mappings.json?t=${Date.now()}`)
      .then(r => r.ok ? r.json() : {})
    ...
    try {
      ...
      if (!put.ok) {
        ...
        if (put.status === 401 || put.status === 403) {
          try { localStorage.removeItem("quasi_dashboard_gh_pat"); } catch {}
  ...
  // Safe localStorage helpers — the github.io origin is shared with any other
  // Pages site on this account, which can fill the ~5MB quota and trigger
  ...
  if (!manifest) return <div style={{ padding: 24, fontFamily: "sans-serif", color: "#8a6f68" }}>Loading Triple Whale data…</div>;

  const ALL_DIMS_LABELS = {
    format: "Format", ad_type: "Ad Type", icp: "ICP", problem: "Concept/Problem",
    creative_no: "Creative No.", agency: "Strategist", batch_name: "Batch",
  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", background: "#FAF6F2", minHeight: "100vh", padding: "32px 24px" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 20 }}>
              {[
                ...
                { label: "Total Revenue", value: fmt(totals.rev), sub: "Meta channel-reported" },
        ...
        {tab === "Strategists" && (
          <div style={{ background: "#fff", borderRadius: 10, padding: 16, border: "1px solid #E8DCD0", overflowX: "auto" }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, margin: "0 0 12px", color: "#3a2429" }}>Strategist Performance ({agencies.length} strategists) · <span style={{ fontWeight: 400, color: "#b0978f" }}>click a row to drill into ads</span></h3>
            <BreakdownTable rows={agencies} nameLabel="Strategist" onRowClick={r => openModal(`Strategist: ${r.name}`, `${r.creatives} ads · Spend ${fmt(r.spend)}`, r.ads)} />
          </div>
        )}
